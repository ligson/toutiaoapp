...
import 'package:toutiao/tt/article/ArticlePage.dart';
...
class ImageListItem extends StatefulWidget {
  createState() => new _ImageListItemState(article);
...
class _ImageListItemState extends State<ImageListItem> {
  ...
  _ImageListItemState(this.article);

  void _onPress() {
    Navigator.of(context).push(new MaterialPageRoute(builder: (_) {
      return new ArticlePage(article);
    }));
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget titleSection = new FlatButton(
        padding: const EdgeInsets.all(5.0), child: row, onPressed: _onPress);
    //...
    return titleSection;
  }
}
